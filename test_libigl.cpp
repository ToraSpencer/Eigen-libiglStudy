...
{ 
	...
	void test1()
	{
		...
		objReadMeshMat(vers, tris, objFilePath.c_str());
		...
		debugWriteMesh("AABB", boxVers, boxTris); 
		{
			//			栅格数据的分布：
			/*
				按索引增大排列的栅格中心点为：
		...
		debugWriteMesh("scaledMesh", versResult_SDF, trisResult_SDF); 
		...
		debugWriteMesh("scaledMeshOri", versResult_SDF, trisResult_SDF); 
