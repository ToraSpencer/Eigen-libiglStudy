    ...
    float lb, float rb, float step)
{
    ...
    { 
            ...
            A(i, i - 1) = diff_x[i - 1];
            B(i, 0) = 3.0 / diff_x[i] * \
                (D_vec[i + 1] - D_vec[i]) - 3.0 / diff_x[i - 1] * (D_vec[i] - D_vec[i - 1]);
    }
    ...
    for (int i = 0; i < m - 1; i++)
    {
        A_vec[i] = (B_vec[i + 1] - B_vec[i]) / 3.0 / diff_x[i];
        C_vec[i] = (D_vec[i + 1] - D_vec[i]) / diff_x[i] - B_vec[i] * diff_x[i] - A_vec[i] * diff_x[i] * diff_x[i];
    }
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        float y = D_vec[curr] + C_vec[curr] * X + \
            B_vec[curr] * X * X + A_vec[curr] * X * X * X;
        result[i] = y;
    }
