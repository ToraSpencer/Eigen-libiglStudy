...
// genCylinder()重载4——输入平行的上底面和下底面的2D边界环路，生成柱体：
template <typename T, typename DerivedVa, typename DerivedVt, typename DerivedVb>
bool genCylinder(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& vers,\
	Eigen::MatrixXi& tris, const Eigen::PlainObjectBase<DerivedVa>& axisVers,\
	...
	const Eigen::PlainObjectBase<DerivedVb>& btmLoop, const bool isCovered)
{
	...
	*/
	...
	vers.resize(0, 0);
	tris.resize(0, 0);
	const int circVersCount = topLoop.rows();						// 一圈横截面环路的顶点数；
	const int circCount = axisVers.rows();								// 圈数；
	const int versCount = circVersCount * circCount;
	std::vector<MatrixXT> circuitsVec(circCount);							// 每个横截面的一圈顶点；
	std::vector<RowVector3T> sectionNorms(circCount);				// 每个横截面的法向；
	...
	if (isCovered)
	{
		...
		for (int i = 0; i < capTrisTop.rows(); ++i)
		{
			int tmp = capTrisTop(i, 2);
			capTrisTop(i, 2) = capTrisTop(i, 1);
			capTrisTop(i, 1) = tmp;
		}
...
// genCylinder()重载5——输入不平行的上底面和下底面的3D边界环路 ，生成柱体：
...
*/
	...
	using RowVector3O = Eigen::Matrix<ScalarO, 1, 3>;
	const int circVersCount = topLoop.rows();							// 一圈横截面环路的顶点数；
	const int circCount = 2 + layersCount - 1;							// 圈数；
	const int versCount = circVersCount * circCount;
...
template <typename T>
bool genAlignedCylinder(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& vers, \
	Eigen::MatrixXi& tris, 	const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& axisVers, \
	const std::pair<float, float> sizePair, const float SR, 	const bool isCovered = true)
{
	/*
	bool genAlignedCylinder(
			Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& vers, \
			Eigen::MatrixXi& tris, 	
			const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& axisVers, \			轴线
			const std::pair<float, float> sizePair,																柱体横截面长宽
			const float SR, 																								柱体侧面上每圈点的采样步长；
			const bool isCovered = true																			是否加盖
			)
	...
	*/
	...
	for (unsigned i = 0; i < circCount - 1; ++i)
	{
		...
		getRotationMat(rotation2, RowVector3T{ 0, 1, 0 }, sectionNorms[i]);
